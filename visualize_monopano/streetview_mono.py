...
class PanoramaMetadata:
        ...
        def computeDepthMap(self):
            ...
            width = int(self.ImageWidth)/26 #360deg
            height = int(self.ImageHeight)/26
            ...
            # panorama is 13312*6656 we can downsample the depth image size by divide 26
            # this is just for computing efficiency
...
pano = GetPanoramaMetadata(lat=48.801386, lon=2.13162, radius=12)


#summer: 48.801507 2.131599; lat=48.801386, lon=2.13162,
...
resized_image = cv2.resize(panoRGB, (512, 256)) 
RGBsmall = im.fromarray(resized_image)

RGBbig = im.fromarray(panoRGB)
        
path ='./result/'
savePanoName = 'pano.png'
savePanoName= os.path.join(path,savePanoName)
#RGBsmall.save(savePanoName) 
RGBbig.save(savePanoName)      
        
print "Save the panorama RGB saved."
...
depthFinal = numpy.zeros(shape=pano.depthMap.shape, dtype=numpy.float) #unit 8 #set to float to high precision
heightD = depthFinal.shape[0]
widthD = depthFinal.shape[1]

saveDepthName ='depth_uint.png'
saveDepthName2 ='depth_normalize.png'

numpy.savetxt('./result/depth.csv', pano.depthMap)

saveDepthName = os.path.join( path, saveDepthName)

# 200 is a depth scale factor
cv2.imwrite(saveDepthName,(pano.depthMap*200).astype(numpy.uint16))

cv2.imwrite(saveDepthName2,pano.depthMap)
        

# visualize main depth planes
plane  = numpy.array(pano.DepthMapIndices)

plan = plane.reshape((256,512))

plan *= 255/plan.max()    # Uses 1 division and image.size multiplications

import matplotlib.pyplot as plt
from matplotlib import cm as cm

img = plt.imshow(plan, interpolation='nearest')
img.set_cmap('gist_ncar')
plt.axis('off')
plt.savefig("./result/plan.png", bbox_inches='tight')
